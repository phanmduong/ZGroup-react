...
export default class SmsList extends Component {
    ...
    componentWillMount() {
        ...
        api.getSmsList().then(function (res) {
            this.setState({smsList: [...this.state.smsList, ...res.data], isLoading: false});
        }.bind(this));
    }
    ...
    loadMore() {
        ...
        api.getSmsList(this.state.page + 1).then(function (res) {
            this.setState({smsList: [...this.state.smsList, ...res.data], isLoading: false, page: this.state.page + 1});
        }.bind(this));
    }
            ...
            <div>
                <table className="striped">
                    ...
                    <tr>
                        <th>Tên</th>
                        <th>Số điện thoại</th>
                        <th style={{maxWidth: "500px"}}>Nội dung</th>
                        <th>Thời gian gửi</th>
                        <th>Trạng thái</th>
                        <th>Mục đích</th>
                    </tr>
                    ...
                    <tbody>
                    {
                        this.state.smsList.map(sms => (
                                <tr key={sms.id}>
                                    <td>{sms.name}</td>
                                    <td>{sms.phone}</td>
                                    <td style={{maxWidth: "500px"}}>{sms.content}</td>
