...
function uploadServer() {
    ...
    console.log(data);
    $.post("/landing-page-build/export", data, function (data, status) {
        $("#exportSubmit").html("<i class=\"pi pixicon-download\"></i> Xuất");
        $('#exportModal').modal('toggle');
        $("#exportModalSuccess").modal('show');
        $("#open-link-landingpage").attr("href", data.url);
    });
}
    ...
    if ($('#styleEditor').css('left') == '0px') {
        ...
        $('#pageList ul li iframe').each(function () {
            ...
            for (var key in editableItems) {
                ...
                $(this).contents().find('body' + ' ' + key).unbind('mouseenter mouseleave click').css({
                    'outline': '',
                    'cursor': ''
                });
