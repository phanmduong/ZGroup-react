        ...
        <div class="col-md-8 ">
            <ul class="pagination pagination-primary" >
                ...
                @if($orders->currentPage() != 1)
                    <li class="page-item">
                        <a class="page-link" href="http://nhatquangshop.test/manage/orders?page=1" aria-label="Previous">
                ...
                @for($i = 1; $i <= $orders->count(); $i++)
                    @if($orders->currentPage() == $i)
                        <li class="active page-item"><a class="page-link" href="http://nhatquangshop.test/manage/orders?page={{$i}}">{{$i}}</a></li>
                    @else
                        <li class="page-item"><a class="page-link" href="http://nhatquangshop.test/manage/orders?page={{$i}}">{{$i}}</a></li>
                    @endif
                @endfor
                @if($orders->currentPage() != $orders->count())
                    <li class="page-item">
                        ...
                        <li class="page-item">
                            <a class="page-link" href="http://nhatquangshop.test/manage/orders?page={{$orders->count()}}" aria-label="Next">
