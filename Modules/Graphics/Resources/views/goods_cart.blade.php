    ...
    @foreach($books as $book)
        <div class="row" id="book-{{$book->id}}" style="margin-bottom:20px;">
            ...
            <div class="col-md-3 h-center">
                <button onclick="removeItem({{$book->id}}, {{$book->price * $book['coupon_value']}})"
                        class="btn btn-success btn-just-icon btn-sm">
                    <i class="fa fa-minus"></i>
                </button>
                &nbsp
                <button onclick="addItem({{$book->id}},{{$book->price * $book['coupon_value']}})"
            ...
            <div class="col-md-2 h-center">
                <p>{{currency_vnd_format($book->price * $book['coupon_value'])}}</p>
            </div>
            <div class="col-md-2 h-center">
                <p>
                    <b style="font-weight:600;" id="book-{{$book->id}}-price"
                       data-price="{{$book->price * $book['coupon_value'] * $book->number}}">
                        {{currency_vnd_format($book->price * $book['coupon_value'] * $book->number)}}
                    </b>
                </p>
            </div>
        </div>
