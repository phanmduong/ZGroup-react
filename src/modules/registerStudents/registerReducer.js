...
export default function registerReducer(state = initialState.registerStudents, action) {
    switch (action.type) {
        ...
        case types.BEGIN_LOAD_BASE_FILTER:
            return {
                ...state,
                ...{
                    isLoadingBaseFilter: true,
                }
            };
        case types.LOAD_BASE_FILTER_SUCCESS:
            return {
                ...state,
                ...{
                    isLoadingBaseFilter: false,
                    baseFilter: action.filter
                }
            };
        case types.LOAD_BASE_FILTER_ERROR:
            return {
                ...state,
                ...{
                    isLoadingBaseFilter: false,
        ...
        case types.CONFIRM_CHANGE_CLASS_REGISTER_STUDENT_SUCCESS:
            registers = changeClassRegister(action.registerId, action.class, state.registers, action.code);
            return {
                ...state,
                ...{
...
function changeClassRegister(registerId, classData, registers, code) {
    if (registers) {
        registers = registers.map(register => {
                if (register.id === registerId) {
                    return {
