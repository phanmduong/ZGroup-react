...
import {historyCheckinoutShiftApi} from "./dashboardCheckinCheckoutApi";
...
export default class DashboardHistoryWorkShiftStore {
    ...
    loadClasses = (filter) => {
        this.isLoading = true;
        historyCheckinoutShiftApi(filter).then((res) => {
    ...
    get shifts() {
        let data = this.history.map((item) => {
            let users = item.users.map((user) => {
                let shifts = user.shifts.map((shift) => {
            ...
            users = users.filter((item) => item.shifts.length > 0);
            return {
                ...item,
                users: users
            };
