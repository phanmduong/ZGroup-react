...
class DashboardTrongDongContainer extends Component {
		...
		let registersData = registers.map((register) => {
			...
			if (register.similar_room) {
				...
				store.rooms.map((room) => {
					const arrRooms = register.similar_room.filter((room_id) => room.id == room_id);
	...
	export = () => {
		...
		let wb = newWorkBook();
		let cols = renderExcelColumnArray([ 5, 25, 10, 25, 20, 25, 25, 25, 15, 20 ]); //độ rộng cột
		...
		data = data.reverse().map((item, index) => {
			...
			let res = {
				STT: index + 1,
				'Khách hàng': item.user.name || 'Không có',
				'Số điện thoại': item.user.phone || 'Không có',
				'Địa chỉ': item.user.address || 'Không có',
				Email: item.user.email || 'Không có',
