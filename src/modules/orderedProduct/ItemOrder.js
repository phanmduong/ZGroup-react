...
class ItemOrder extends React.Component {
	...
	isCheckedUser(deliveryId) {
		const deliveries = this.props.chosenItems.filter((delivery) => delivery.id === deliveryId);
		if (deliveries.length > 0) return this.props.isAll || !!deliveries[0].checked;
		return false;
	}
			...
			<tr>
				{this.props.isSendingPrice && (
					<td>
						<input
							type="checkbox"
							checked={this.isCheckedUser(delivery.id)}
							onChange={(e) => this.props.chooseItem(delivery.id, e.target.checked)}
						/>
					</td>
				)}
				...
				<td style={{ padding: '0 5px' }}>
					{delivery.customer ? (
						<span>
							{delivery.customer.name || ''}&nbsp; ({delivery.customer.phone || ''})<br />
				...
				<td style={{ padding: '0 5px' }}>
					<a href={attach_info.link} target="_blank">
						{link}
					</a>
				</td>
				<td>{attach_info.size}</td>
				<td>{attach_info.color}</td>
				<td style={{ padding: '0 5px' }}>{attach_info.code}</td>
				...
				<td style={{ padding: '0 5px' }}>
					<StatusSelect options={ORDERED_STATUS} onChange={this.changeStatus} value={delivery.status} />
				</td>
				<td style={{ padding: '0 5px' }}>
					<a type="button" onClick={() => this.props.showAddNoteModal(delivery)}>
						{delivery_note === '' ? <i className="material-icons">edit</i> : <p>{delivery_note}</p>}
					</a>
				</td>
				<td style={{ padding: '0 5px' }}>
